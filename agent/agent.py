...
class Agent:
    ...
    def decide(self,
        ...
        # STM 完全空時（剛睡醒第一個 tick），直接跟排程走，不隨機抽行動
        # T002+ 讓正常邏輯運作（STM=1 時均勻分布→U≈1.0→觸發 deliberate）
        if len(stm_turns_for_u) < 1:
            slot      = char.get_current_slot()
            sched_loc = slot.get("location", "") if slot else ""
            cur_loc   = char.current_location or ""
            # 字串包含比對：「公寓」 in 「公寓三樓」 → 視為同地點
            at_place  = bool(sched_loc) and (
                sched_loc in cur_loc or cur_loc in sched_loc
            )
            if sched_loc and not at_place:
                verb, target = "前往", sched_loc
            else:
                verb, target = "休息", ""

            action = f"{verb}:{target}" if target else verb
            ...
            self.stm.add_turn(turn_id=turn_id, scene=scene,
                              image_desc=image_desc, input_text=input_text,
                              action=action, ham_propositions=[])

            confusion = {"U": 1.0, "K": 0.0, "S": 0.0,
                         "C": char.get_confusion_weights().get("w1", 0.4),
                         "mode": "intuitive"}
            return {
                "action": action, "verb": verb, "target": target,
                "thought": "", "ham": [], "mode": "markov",
                "action_probs": {}, "confusion": confusion,
    ...
    def _decide_markov(self, scene: str, co_located_names: list,
        ...
        # 若是「前往」則從時間表取目標地點；若無排程則用角色職業地點
        if verb == "前往" and not target:
            target = self._next_schedule_location() or self._default_work_location() or ""
