...
def reset_character(code: str):
    """
    將指定角色的可變狀態重置為乾淨初始值。
    保留靜態設定（個性、role、relationships.initial 等）。
    """
    ...
    data["state"]["current_location"] = data["state"].get(
        "initial_location", list(data.get("residence", {}).values() or ["公寓"])[0]
        if isinstance(data.get("residence"), dict) else "公寓"
    )
    ...
    data["schedule"]["slots"] = [
        s for s in data["schedule"]["slots"]
